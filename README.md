...
**Chapter 10's four questions are the first chapter quizzes written to be hard**: four choices instead of three, and each one needs judging between plausible options or unlearning something. Two of the wrong answers are the things people actually say, "berat badan saya stabil kok" and "turunin berat dulu, latihan nanti", and the last question asks what you would tell a 55-year-old parent who thinks it is too late. `test_the_newest_chapter_asks_harder_questions` holds whatever the newest chapter is to that bar. Note that changing a question's number of choices moves where its answer lands, so answers already recorded against it stop lining up.
...
- **No emoji inside a chapter.** They read as machine-written, which is exactly the wrong impression for health advice somebody has to trust. Emoji live on the index tiles only, where they work as icons. `chapter["emoji"]` exists for that; blocks have no emoji field.
...
- The result screen always offers a way back to the chapter list, next to the next-chapter button, so finishing a chapter is never a dead end.
...
- **Rotation, not a calendar**: `(day - ROTATION_EPOCH) % how many are active`, ordered by `position`. Everyone gets the same question on the same day, it never runs out, and it loops once it reaches the end (115 questions today, so a 115-day cycle).
- **`position` is not the file order** (`nutrition/interleave.py`). `daily_questions.py` groups questions by topic so a human can review a hundred of them, and the rotation used to walk that order, which turned every group into a run: twelve straight days of calories, then twelve of protein, then ten of drinks, with weight training not due until day 55. A member said it read like the same question every morning, and the file agreed with them. So the order is computed and written into `position` by a migration: each topic dealt out at an even stride, starting from its own hashed offset, then a repair pass to separate the few that still land together. Two properties, both tested: no two days in a row share a topic, and any seven days touch at least five. Hashed rather than randomised, like `shuffle.py`, so every machine and every deploy agree forever.
- **Reordering only ever touches the days nobody has had yet.** A question that has been shown keeps its slot: move today's out from under somebody who has already answered it and they get a new question with their old answer marked on it, until midnight. `0006_spread_daily_topics` freezes positions 1 to 30 (the rotation had reached day 30 when it was written) plus anything with a recorded answer, in case it reaches production later than intended, and deals the rest into the slots that are left. Reordering again means bumping `FROZEN_THROUGH` to whatever position the rotation has reached by then. One consequence left standing: the first 30 days replay in their original clumped order when the cycle wraps in late November.
- **Adding a batch** means appending to `daily_questions.py` with a `topic`, a migration calling `sync_questions`, and, if it is worth spreading rather than sitting at the end of the queue, a second migration interleaving the unshown tail again.
...
- **Every question from `dq-007` on has to make somebody think.** The set as first written was answerable in about five seconds: three choices, and usually the one plausible number in the middle. `dq-101` onwards set the bar, and `0005_harder_daily_questions` brought `dq-007` to `dq-100` up to it. That bar is **four choices** rather than three (a blind guess drops from 33% to 25%) and an answer that needs working out (do the arithmetic), judging (all four are true, which matters most?), or unlearning something (the intuitive answer is the wrong one). Hard is not the same as obscure: no trivia, no trick wording, never two defensible answers. The numbers reused are the ones the chapters and the first six questions teach, so the harder questions put them to work rather than adding more to remember. `DailyDifficultyTest` holds every question past `dq-006` to four choices and to explanations long enough to carry a reason.
- **`dq-001` to `dq-006` stay as they were**, three choices and answerable in five seconds, because members have already answered them. A recorded answer is only a letter, so changing the choices under one leaves that letter pointing at different text and mixes two different questions into one all-time split.
- **Rewriting a seeded question needs `rewrite_questions` in a migration**, since `sync_questions` never overwrites and editing the file alone does nothing to a database that has already been seeded, with nothing failing anywhere to say so. `0005` is the pattern: it passes only the codes nobody has answered yet, skips (and names, in the deploy log) any that have been, and leaves `position` alone so the rotation does not move. A question that has to be skipped needs a **new code** rather than another rewrite. `DailyQuestionSeedTest` compares every seeded row against the module, which is the check that catches an edit that never reached the database.
- The answer-placement guardrail is grouped by choice count, since mixing three- and four-choice questions into one tally would hide a lopsided group. It only holds a group to the spread once that group is at least four questions per slot: the six three-choice leftovers land 4/1/1, which is noise at that size, not a letter worth guessing.
